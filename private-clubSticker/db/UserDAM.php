...
class UserDAM extends DAM {
    ...
    public function writeUser($user) {
        ...
        if ($userDB == null) {
            ...
            $query = 'INSERT INTO users
                (userID, lastName, firstName, phone, password, description, profilePicture)
              VALUES
                (:userID, :lastName, :firstName, :phone, :password, :description, :profilePicture)';
            $statement = $this->db->prepare($query);
    ...
    private function bindValues($user, $statement) {
        ...
        $statement->bindValue(':description', $user->description);
        $statement->bindValue(':profilePicture', $user->profilePicture);
    }
}
