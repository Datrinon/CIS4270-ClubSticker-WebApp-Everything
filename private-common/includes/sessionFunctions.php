...
// session.cookie_lifetime = 0 (cookie expires on browser close -- useful for banking apps)
// session.cookie_secure = 1 (cookies will only be transmitted over ssl connections)
// session.cookie_httponly = 1 (cookies can only be accessed by http protocol)
// session.use_only_cookies = 1 (sessionid cannot be reached from GET / POST requests)
...
function last_login_is_recent() {
    ...
    //$max_elapsed = 10 * 1; // 1 minute for debugging
...
// Is user logged in already? for use in confirm_user_logged_in.
...
#region header redirection content
...
//    $vm = null;
    ...
    require(APP_NON_WEB_BASE_DIR . INVALID_SESSION_PAGE); // INVALID_SESSION_PAGE loads a form.
    exit;
}
